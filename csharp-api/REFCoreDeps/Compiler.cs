    ...
    public class Compiler
    {
        ...
        static public byte[] Compile(string filepath, Assembly executingAssembly, List<Assembly> deps)
        {
            ...
            using (var peStream = new MemoryStream())
            {
                ...
                if (!result.Success)
                {
                    REFrameworkNET.API.LogInfo("Compilation done with error.");
                ...
                REFrameworkNET.API.LogInfo("Compilation done without any error.");
        ...
        public static List<PortableExecutableReference> GenerateExhaustiveMetadataReferences(Assembly executingAssembly, List<Assembly> deps) {
            ...
            if (Directory.Exists(baseRefDir)) {
                ...
                REFrameworkNET.API.LogInfo("Looking for reference assemblies in " + refFolder);

                if (Directory.Exists(refFolder)) {
                    REFrameworkNET.API.LogInfo("Found reference assemblies in " + refFolder);
                    dlls = Directory.GetFiles(refFolder, "*.dll");
                }
            } else {
                REFrameworkNET.API.LogInfo("No reference assemblies found in " + baseRefDir);
            }
            
            if (dlls.Length == 0) {
                REFrameworkNET.API.LogInfo("No reference assemblies found in " + baseRefDir + ". Falling back to implementation assemblies in " + assemblyPath);
            ...
            referencesStr.RemoveWhere(r =>
            {
                try
                {
                    ...
                    if (!mr.IsAssembly)
                    {
                        REFrameworkNET.API.LogInfo("Removed reference: " + r);
                        return true;
                    }
                }
                catch (Exception)
                {
                    REFrameworkNET.API.LogInfo("Error adding reference: " + r);
                    //REFrameworkNET.API.LogInfo(e);
                    return true;
                }
        ...
        private static CSharpCompilation GenerateCode(string sourceCode, string filePath, Assembly executingAssembly, List<Assembly> deps)
        {
            ...
            {
                REFrameworkNET.API.LogInfo(reference.Display);
            }
